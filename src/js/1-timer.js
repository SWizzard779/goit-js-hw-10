...
const options = {
    ...
    onClose(selectedDates) {
        ...
        if (selectedDate && selectedDate.getTime() > new Date().getTime()) {
            ...
            calendarElement.disabled = false; // Ensure the picker is re-enabled for correction
            iziToast.error({
                title: 'Error',
...
button.addEventListener('click', (event) => {
    ...
    calendarElement.disabled = true; // Disable the picker to prevent changes
    button.disabled = true; // Keep the button disabled until a new valid date is selected
    ...
    timerInterval = setInterval(() => {
        ...
        if(diff <= 0) {
            ...
            calendarElement.disabled = false; // Re-enable the picker for a new selection
        } else {
